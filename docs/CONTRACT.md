...
割り算の分母(1 環境がリスナールールとターゲットグループをいくつ取るか)は
**`template.yaml` に書かれている個数をそのまま数える**。複数サービスの環境は
サービスの数だけ両方を作るので、固定値にすると残り面数が N 倍に出る(#188)。
テンプレートが読めないときは単一サービスと仮定し、**仮定であることを出力に書く**
(`--json` では `per_environment.source` が `template` か `assumed` になる)。
...
サービスの Index を **連結**する(CloudFormation に算術が無いため)。

```
Priority = "${EnvRulePriority}${Index}"
```

この形には 2 つの制約がある(#187)。

**Index は 1 桁でなければならない。** 連結が単射なのは桁数が揃っているときだけで、
2 桁になると別の環境と衝突する:
...
そのため **1 環境あたりのサービスは 10 個まで**(`scaffold.MaxServices`)。
超えると `init` が理由つきで失敗する。

**base は 1〜4999 に収める。** PR 番号をそのまま使うと 5001 で
`"5001" + "0"` = 50010 となり上限を超えて `CREATE_FAILED` になる。生成される
workflow は `PR % 4999 + 1` で畳む。

畳んだ結果として、**同時に開いている環境の PR 番号がちょうど 4999 離れると衝突する**。
