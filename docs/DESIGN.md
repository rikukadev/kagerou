...
> **既定は per-app(§10.2)。** 共有はオプトインの選択肢であって、
> 推奨ではない。理由は §11.3。

per-app ベース(§10.2)はアプリごとに CloudFront・証明書・DNS を作る。
**1 ドメインを全アプリで共有したい**場合のために `Mode: shared` を用意する。
...
### 11.3 なぜ per-app が既定か

実機で両方を動かした上での結論(2026-09-13):

- **アプリ開発は互いに独立した作業**。ベースを共有すると、片方の更新が
  もう片方の配信に影響しうる(CloudFront の設定・CF Function・証明書は
  1 つの CFN スタックに同居する)。per-app なら爆発半径がリポジトリに閉じる
- **所有が自然**。ベースのテンプレートはアプリのリポジトリにあり(#77)、
  そのリポジトリのコラボレータが再適用できる。共有ベースは「誰のものか」が
  曖昧になり、更新の調整が要る
- **コスト差がほぼ無い**。CloudFront は従量課金で固定費なし、ACM は無料、
  Route53 レコードは事実上ゼロ。ベースが増えても費用はほとんど増えない
  (増えるのは初回の証明書検証 + 配信作成の待ち時間 ≒ 15 分だけ)
- URL も per-app の方が読みやすい(`pr-42.todo.example.com`)

共有が効くのは「1 アカウントに数十以上のアプリがあり、ディストリビューション数や
ドメイン運用を一元管理したい」場合。その時だけ `Mode: shared` を選ぶ。

## 12. スコープ境界: kagerou は preview 専用(本番は mahoroba)

kagerou が扱うのは **PR / E2E / デモで使い捨てる環境だけ**。本番環境の管理は
姉妹ツール **[mahoroba](https://github.com/rikukadev/mahoroba)**(別物)の仕事で、
kagerou に本番向けの機能を足すことはしない。

名前も対にしてある。陽炎は**あるように見えて消えるもの**、まほろばは
**住むに足る良い場所**。どちらを触っているかが名前で分かる。
...
- `ttl: none` は「デモを数日残す」ための明示的な例外であって、**本番運用の入口ではない**。
  本番をここに載せたくなったら、それは mahoroba の仕事
- 承認フロー・段階的 rollout・アラーム連動・drift 検出は kagerou には入れない

### mahoroba と共有するもの(この境界のために切り出してある)
...
- **Production** — mahoroba の read API(将来)を読む
...
## 13. 入口の選択: なぜ ALB を標準にしたか(Lambda@Edge の実証つき)

入口は **compute(何が動くか)× entrypoint(どう公開するか)** の 2 軸で決まる。
結論は **ALB を標準**とし、他は条件つきの選択肢に留める。判断の軸は §10 で
既に決めた「**URL は常に Host ベース**(パス分けはアプリの挙動に影響が
大きいので採らない)」。

| compute × 入口 | ルーティングの置き場所 | 複数サービスの環境 | 固定費 | 上限 |
|---|---|---|---|---|
| **lambda × alb(標準)** | **インフラの宣言**(リスナールール = CFN リソース)。host でも path でも書ける | `api-pr-42` / `web-pr-42` のように **host で分けられる**。本番の ALB 構成と同じ形 | 共有 ALB 1 本(月 18 ドル前後) | なし |
| lambda × apigateway | API ごとに固有 | ホストが増え **URL 構造が一定しない**。独自ドメインですらない | ゼロ | 応答 30 秒 |
| ecs × alb | 同上(ALB) | host で分けられる | 共有 ALB 1 本 | なし |
| **ecs × apigateway** | HTTP API のルート。タスクは Cloud Map に **SRV** で登録し VPC Link で引く | 環境ごとに API を持つので**優先度の採番も衝突も無い** | ゼロ(VPC Link は時間課金なし) | 応答 30 秒 |
| static + Lambda@Edge | **関数のコード**(環境変数を持てないので SSM から読む) | 1 ホストしか無いので **パス分割しかない** | 実質ゼロ | オリジン応答 〜60 秒 |

**固定費ゼロでも環境全体がゼロになるわけではない**(#105)。`ecs` は Fargate が
動いている限り課金されるので、消えるのは**入口の固定費だけ**。だから
`ecs × apigateway` の生成物は TTL を 24h に寄せてある(lambda 形は 72h)。

### Lambda@Edge は「動く」ことを実証済み(2026-09-14)

捨てプロトタイプ(`kagerou-edge-proto`、削除済み)で次を確認した:

- CloudFront + Lambda@Edge(origin-request)で **1 ホストの `/` と `/api/*` を
  別々の Lambda に振れる**(GET/POST とも 200)
- **罠**: origin を差し替えても Host ヘッダは元のままなので、
  `request.headers.host` を新しい origin に揃えないと API Gateway が 403 を返す。
  CloudFront 側は `AllViewerExceptHostHeader` を使う(viewer の Host を流すと壊れる)
- Lambda@Edge は環境変数を持てず、更新に replication 待ちがある

つまり技術的な障害があって不採用にしたのではない。**ホストで分けられない**という
構造の問題で標準から外した。次のときに同じ検討を繰り返さないよう、事実として残す。

### 使い分け

- **既定**: ALB(`--entrypoint alb`)。独自ドメインで配り、複数サービスは host で分ける
- **固定費ゼロでコンテナを動かす**: `--compute ecs --entrypoint apigateway`(#105)。
  ALB と同じコンテナを動かせて月 18 ドルが消える。引き換えに応答 30 秒の上限が付くので、
  長い処理や WebSocket を持つアプリでは選べない
- **固定費ゼロが要る(静的)**: static + 認証なら Lambda@Edge が候補
  (パス分割を受け入れられる場合に限る)
- **ドメインが無い**: API Gateway の生 URL に自動フォールバック

### ecs × apigateway で分かったこと(#105、実機検証つき)

`e2e/aws/apigw` が実 AWS で毎 PR 回っている。#103(ALB 版)との差分:

- **入口の固定費は消える。** VPC Link は HTTP API では時間課金が無い。ただし
  Fargate は動く限り課金されるので、アイドル 0 円にはならない
- **衝突の採番が要らない。** ALB は共有リスナー上でリスナールールの優先度を
  一意にする必要があり、CI が PR 番号を渡している。API Gateway は環境ごとに
  API を持つので、その採番自体が存在しない
- **Cloud Map は SRV を要求する。** A レコードで登録すると cfn-lint も通り
  デプロイも成功して、**実行時に 503 になる**。SRV にはポートが要るので
  ECS の `ServiceRegistries` に `ContainerName` / `ContainerPort` が要る
- **driver の追加は不要。** #103 と同じく素の CloudFormation で書ける

## 14. 生成物のバージョン印(#207)

`kagerou init` が書いたファイルは、書いた瞬間から**利用者のファイル**になる。
こちらでバグを直しても既存のリポジトリには届かない。実例:

- リスナー優先度の式(`PR % 4999 + 1`)は**生成された workflow / template の中**にある。
  v0.11 以前に init したリポジトリは、5000 番台の PR で `CREATE_FAILED` のまま
- apigw ベースの Cloud Map を A → SRV に直した判断も、生成済みのファイルには反映されない

一番まずいのは**黙って古いまま動いていること**なので、最低限
「いつの kagerou が書いたか」と「それ以降に何を直したか」は言えるようにした。

```
# generated by kagerou v0.13.0 — 再生成は `kagerou init --force`(手を入れたファイルも上書きされる)
```

- 印は `internal/genstamp`。テンプレート関数 `generatedBy "<コメント記号>"` で先頭に入る
- `kagerou validate` が生成物の印を読み、古ければ**踏む問題を名指しする**
  (「古い」だけでは何をすればいいか分からない)
- **error にはしない**。生成物は利用者が手を入れている可能性があり、
  こちらの都合で CI を落とす筋合いがない
- 開発ビルド(`version = dev`)では古さを判定しない。毎回警告が出ると誰も読まなくなる

### 生成物の挙動を変えたら `genstamp.Fixes` に足す

これが運用上の肝。**生成されるファイルの挙動が変わる修正**を入れたら、
`internal/genstamp` の `Fixes` に 1 件足す(現象 → 影響するファイル)。
足さないと、古い生成物を持つ利用者には何も伝わらない。文言の調整や整形は対象外。

### ここで止めた理由(`kagerou upgrade` をまだ作らない)

再生成の diff を出す(`upgrade --check`)、3-way マージする(`upgrade`)まで行くと
「利用者の変更をどう保つか」を決める必要がある。`template.yaml` は**手を入れる前提**の
ファイルなので、マージを誤ると本人の変更を消す。まず「気づける」ところまでを入れて、
どのファイルが実際に手で変更されるかを見てから次を決める。
