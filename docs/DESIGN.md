...
環境を公開する入口は 4 つ考えられる。結論は **ALB を標準**とし、他は条件つきの
選択肢に留める。判断の軸は §10 で既に決めた「**URL は常に Host ベース**
(パス分けはアプリの挙動に影響が大きいので採らない)」。

| 入口 | ルーティングの置き場所 | 複数サービスの環境 | 固定費 |
|---|---|---|---|
| **ALB(標準)** | **インフラの宣言**(リスナールール = CFN リソース)。host でも path でも書ける | `api-pr-42` / `web-pr-42` のように **host で分けられる**。本番の ALB 構成と同じ形 | 共有 ALB 1 本(月 18 ドル前後) |
| Lambda@Edge | **関数のコード**(環境変数を持てないので焼き込み) | 1 ホストしか無いので **パス分割しかない** | 実質ゼロ |
| API Gateway カスタムドメイン | API ごとに固有 | サービス数だけホストが増え、**URL 構造が一定しない** | ゼロ |
| API Gateway 生 URL | — | 同上(しかも独自ドメインですらない) | ゼロ |
...
- **固定費ゼロが要る**: 単一サービス、または static + 認証なら Lambda@Edge が候補
  (パス分割を受け入れられる場合に限る)
